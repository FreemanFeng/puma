...
class _Environment(object):
  ...
  def FetchPath(self, dependency, arch=None, device=None):
    #if UC_BUILD
    try:
      status, output = cmd_helper.GetCmdStatusAndOutput(['which', dependency])
      if output:
        return output.replace('\n', '')
    except:
      pass
    path = getResourceLocation()
    path = os.path.join(path, dependency)
    return path
    #else
    # if self._dm is None:
    #   self.Initialize()
  ...
  def PrefetchPaths(self, dependencies=None, arch=None, device=None):
    #if UC_BUILD
    try:
      status, output = cmd_helper.GetCmdStatusAndOutput(['which', dependencies])
      if output:
        return output.replace('\n', '')
    except:
      pass
    path = getResourceLocation()
    path = os.path.join(path, dependencies)
    return path
    #else
...
def getResourceLocation():
  path = os.path.join(getDevilLocation(), 'bin')
  return path
#endif
