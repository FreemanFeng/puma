...
class Fastboot(object):
  ...
  # if UC_BUILD
  def _FindFastboot(self):
    try:
      status, output = cmd_helper.GetCmdStatusAndOutput(['which', 'fastboot'])
      if output:
        return output.replace('\n', '')
    except:
      pass
    path = devil_env.getResourceLocation()
    path = os.path.join(path, 'fastboot')
    return path

    _fastboot_path = lazy.WeakConstant(lambda: self._FindFastboot())
  # else
