...
#if UC_BUILD
def _FindSplitSelect():
  try:
    status, output = cmd_helper.GetCmdStatusAndOutput(['which', 'split-select'])
    if output:
        return output.replace('\n', '')
  except:
    pass
  path = devil_env.getResourceLocation()
  path = os.path.join(path, 'split-select')
  return path

_split_select_path = lazy.WeakConstant(lambda: _FindSplitSelect())
#else
